...
class ExamController extends \BaseController {
    ...
    public function calificacion2($idexamen) {
        $params = Input::all();
        Log::info(__METHOD__ . "-PARAMETROS-CALIFICACION2[" . print_r($params, true) . "]  ID EXAMEN [" . $idexamen . "]");
        //Se crea la evaluacion correspondiente al examen que toma el estudiante
        $eval_student = new Evaluacion();
        $eval_student->alumno = 1;
        $eval_student->examen = $idexamen;
        $eval_student->punteo = 0;
        $eval_student->save();

        foreach ($params as $key => $value) {
            if ($key != "_token") {
                //por cada pregunta crea un nuevo detalle para la respueta correspondiene de dicha respuesta
                $det_evaluacion = new DetalleEvaluacion();
                $det_evaluacion->pregunta = $key;
                $det_evaluacion->respuesta = $value;
                $det_evaluacion->punteo = 0;
                $eval_student->detalle()->save($det_evaluacion);
                Log::info(__METHOD__ . "-****key[" . $key . "]  key[" . $value . "]");
            }
        }
